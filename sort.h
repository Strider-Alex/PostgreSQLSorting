...
void MERGE_SORT_IN_PLACE(SORT_TYPE *dst, const size_t len) {
	...
	for (p = 2; p <= lr; p += 2) {
		...
		while ((p & q) == 0) {
			if (SORT_CMP(dst1[1 - q], dst1[0 - q]) < 0) {
				break;
			}
...
static void INTRO_SORT_RECURSIVE(SORT_TYPE *dst, const size_t left, const size_t right, size_t depth_limit) {
	size_t pivot;
	size_t new_pivot;
	...
	INTRO_SORT_RECURSIVE(dst, left, new_pivot - 1U, depth_limit - 1);
	INTRO_SORT_RECURSIVE(dst, new_pivot + 1U, right, depth_limit - 1);
}
